// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//   O-TECH BOT v4.1 — by Orlando Tech 🇭🇹
//   ESM — Node v18+ — Fichier UNIQUE
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
import makeWASocket, {
...
const ANTI_QUICK_MSGS = [
    "Eyyy Quick! Ou vini kom? Ale Quick papa ou! Bot O-TECH pa fe jwet! 💀🤣",
    "QUICK?! Non non non fre... soti la epi pa janm tounen! 😭🚫",
    "Sa Quick sa! Ou pa we se O-TECH BOT isit? Ale fe wout ou! 😂🤡",
...
const blagues = [
    ...
    "Pourquoi les fantomes ne mentent pas? Parce qu'on voit a travers eux!",
...
const commands = {
    ...
    menu: async (sock, from, msg) => {
        ...
        const text =
            ...
            "╚═══════════════════════════╝\n\n" +
            "*📋 INFO*\n" +
            `▸ ${p}menu ▸ ${p}ping ▸ ${p}botinfo ▸ ${p}uptime ▸ ${p}owner\n\n` +
            "*🛠 MODERATION*\n" +
            `▸ ${p}kick ▸ ${p}add ▸ ${p}promote ▸ ${p}demote\n` +
            `▸ ${p}mute ▸ ${p}unmute ▸ ${p}warn ▸ ${p}resetwarn ▸ ${p}clearwarn\n` +
            `▸ ${p}delete ▸ ${p}tag ▸ ${p}tagadmin ▸ ${p}admins\n` +
            `▸ ${p}groupinfo ▸ ${p}link ▸ ${p}revoke\n` +
            `▸ ${p}kickall ▸ ${p}kickall2 ▸ ${p}promoteall ▸ ${p}demoteall\n` +
            `▸ ${p}bye ▸ ${p}join\n\n` +
            "*📢 BROADCAST & CONTACTS*\n" +
            ...
            `▸ ${p}profil ▸ ${p}setpp ▸ ${p}setprefix ▸ ${p}public\n` +
            `▸ ${p}sudo ▸ ${p}addprem\n\n` +
            "*🐞 BUG*\n" +
    ...
    owner: async (sock, from, msg) => {
        await reply(sock, from, `👑 *Owner*\n\n▸ ${CONFIG.owner}\n▸ wa.me/${CONFIG.ownerNumber}`, msg);
    },
    ...
    kick: async (sock, from, msg, args, sender) => {
        ...
        await reply(sock, from, `✅ *${mentioned.length}* membre(s) expulse(s)!`, msg);
    },
    ...
    add: async (sock, from, msg, args) => {
        ...
        await reply(sock, from, "✅ Membre ajoute!", msg);
    },
    ...
    demote: async (sock, from, msg) => {
        ...
        await reply(sock, from, "⬇️ Admin retire!", msg);
    },

    mute: async (sock, from, msg) => {
        if (!isGroup(from)) return reply(sock, from, "❌ Groupe seulement.", msg);
        await sock.groupSettingUpdate(from, "announcement");
        await reply(sock, from, "🔇 Groupe ferme — admins only.", msg);
    },
    ...
    warn: async (sock, from, msg) => {
        ...
        for (const jid of mentioned) {
            ...
            if (current >= 3) {
                await sock.groupParticipantsUpdate(from, [jid], "remove");
                warnMap.delete(jid);
                await reply(sock, from, `⛔ @${shortNum(jid)} expulse apres 3 avertissements!`, msg);
            } else {
    ...
    resetwarn: async (sock, from, msg) => {
        ...
        await reply(sock, from, "✅ Avertissements reinitialises.", msg);
    },
    ...
    clearwarn: async (sock, from, msg, args, sender) => {
        ...
        for (const p of meta.participants) {
            if (warnMap.has(p.id)) { warnMap.delete(p.id); count++; }
        }
        await reply(sock, from, `✅ *${count}* warn(s) supprimes dans ce groupe.`, msg);
    },
    ...
    revoke: async (sock, from, msg) => {
        ...
        await reply(sock, from, "✅ Lien revoque!", msg);
    },

    // ── KICKALL — membres non-admin seulement ──
    kickall: async (sock, from, msg, args, sender) => {
        if (!isOwner(sender)) return reply(sock, from, "❌ Owner seulement.", msg);
        if (!isGroup(from)) return reply(sock, from, "❌ Groupe seulement.", msg);
        const meta = await sock.groupMetadata(from);
        const botJid = sock.user.id;
        // FIX: exclure le bot, l'owner ET les admins
        const targets = meta.participants.filter(p =>
            !p.admin &&
            !isOwner(p.id) &&
            shortNum(p.id) !== shortNum(botJid)
        );
        if (!targets.length) return reply(sock, from, "❌ Aucun membre non-admin a expulser.", msg);
        await reply(sock, from, `⏳ Expulsion de *${targets.length}* membres non-admin...`, msg);
        let count = 0;
        for (const m of targets) {
            try { await sock.groupParticipantsUpdate(from, [m.id], "remove"); count++; await wait(600); } catch (_) {}
        }
        await reply(sock, from, `✅ *${count}* expulse(s)! Les admins sont restes.`, msg);
    },
    ...
    kickall2: async (sock, from, msg, args, sender) => {
        ...
        await reply(sock, from, `✅ Groupe vide! *${count}* expulse(s).`, msg);
    },
    ...
    demoteall: async (sock, from, msg, args, sender) => {
        ...
        await reply(sock, from, `✅ *${targets.length}* admins retrograded!`, msg);
    },
    ...
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    //  BROADCAST — envoie un DM a TOUS les membres
    //  du groupe actuel
    ...
    broadcast: async (sock, from, msg, args, sender) => {
        ...
        if (!texte.trim()) return reply(sock, from, "❌ Usage: .broadcast Ton message ici", msg);

        const meta = await sock.groupMetadata(from);
        const botJid = sock.user.id;
        const targets = meta.participants.filter(p =>
            shortNum(p.id) !== shortNum(botJid) && !isOwner(p.id)
        );
        ...
        for (const member of targets) {
            try {
                await sock.sendMessage(member.id, { text: broadcastMsg });
                success++;
                await wait(1500); // delai anti-ban WhatsApp
            } catch (_) { failed++; }
        }
    ...
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    //  CONTACTS — exporte tous les membres du
    //  groupe dans un fichier .txt sans chercher
    //  Usage: .contacts
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    contacts: async (sock, from, msg, args, sender) => {
        ...
        content += `\nRESUME\n`;
        content += `  Admins  : ${numAdmin}\n`;
        content += `  Membres : ${numMembre}\n`;
        content += `  Total   : ${members.length}\n\n`;
        content += "Genere par O-TECH BOT\n";
        ...
        try {
            await sock.sendMessage(from, {
                ...
                caption:
                    ...
                    `👮 Admins: *${numAdmin}* | 👤 Membres: *${numMembre}*\n\n` +
                    `_O-TECH BOT_`
            }, { quoted: msg });
        } catch (e) {
            await reply(sock, from, `❌ Erreur envoi: ${e.message}`, msg);
        } finally {
    ...
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    //  LISTGROUPS — liste tous les groupes du
    //  bot et envoie en DM a l'owner
    //  Usage: .listgroups
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    listgroups: async (sock, from, msg, args, sender) => {
        if (!isOwner(sender)) return reply(sock, from, "❌ Owner seulement.", msg);
        await reply(sock, from, "⏳ Recuperation des groupes...", msg);
        try {
            const chats  = await sock.groupFetchAllParticipating();
            const groups = Object.values(chats);
            if (!groups.length) return reply(sock, from, "❌ Aucun groupe trouve.", msg);

            let text = `🏢 *GROUPES DU BOT*\n\n`;
            text += `📊 Total: *${groups.length} groupes*\n`;
            text += "─".repeat(35) + "\n\n";

            for (let i = 0; i < groups.length; i++) {
                const g = groups[i];
                const isAdmin = g.participants.find(p =>
                    shortNum(p.id) === shortNum(sock.user.id) && p.admin
                );
                text += `*${i + 1}.* ${g.subject}\n`;
                text += `   👥 ${g.participants.length} membres\n`;
            ...
            await reply(sock, from, `✅ Liste de *${groups.length} groupes* envoyee en DM!`, msg);
        } catch (e) {
    ...
    block: async (sock, from, msg, args, sender) => {
        ...
        await reply(sock, from, `🚫 *${mentioned.length}* user(s) bloque(s)!`, msg);
    },
    ...
    unblock: async (sock, from, msg, args, sender) => {
        ...
        await reply(sock, from, `✅ *${mentioned.length}* user(s) debloque(s)!`, msg);
    },
    ...
    ban: async (sock, from, msg, args, sender) => {
        ...
        await reply(sock, from, `🚫 *${mentioned.length}* user(s) banni(s)!`, msg);
    },
    ...
    unban: async (sock, from, msg, args, sender) => {
        ...
        await reply(sock, from, `✅ *${mentioned.length}* user(s) debanni(s)!`, msg);
    },
    ...
    lettre: async (sock, from, msg, args) => {
        ...
        if (s.erreurs >= s.max) {
            penduSessions.delete(key);
            return reply(sock, from, `💀 *PERDU!* Le mot etait: *${s.mot}*`, msg);
        }
    ...
    devinette: async (sock, from, msg, args, sender) => {
        const devs = [
            { q: "Je parle sans bouche, j'entends sans oreilles. Qu'est-ce?", r: "echo", a: "L'echo" },
            { q: "Plus je seche, plus je suis mouillee. Qu'est-ce?", r: "serviette", a: "Une serviette" },
            ...
            { q: "Tout le monde me passe dessus mais personne ne m'ecrase. Qu'est-ce?", r: "route", a: "La route" },
        ];
        const d = rand(devs);
    ...
    // FIX: renomme "de" au lieu de "de" avec accent (evite bug terminal)
    ...
    "8ball": async (sock, from, msg, args) => {
        if (!args.length) return reply(sock, from, "❌ Pose une question! Ex: .8ball Vais-je reussir?", msg);
    ...
    pile: async (sock, from, msg) => {
        await reply(sock, from, `🪙 Resultat: *${Math.random()<0.5?"PILE":"FACE"}*`, msg);
    },
    ...
    conseil: async (sock, from, msg) => {
        const conseils = ["Travay di se kle sikse!","Pa janm abandone rev ou!","Aprann yon bagay nouvo chak jou!","O-TECH la pou ou — Innovation constante!"];
    ...
    setpp: async (sock, from, msg, args, sender) => {
        ...
        try {
            ...
            await reply(sock, from, "✅ Photo mise a jour!", msg);
        } catch (e) { await reply(sock, from, `❌ Erreur: ${e.message}`, msg); }
    },
    ...
    setprefix: async (sock, from, msg, args, sender) => {
        ...
        await reply(sock, from, `✅ Prefixe change: *${CONFIG.prefix}*`, msg);
    },

    public: async (sock, from, msg, args, sender) => {
        if (!isOwner(sender)) return reply(sock, from, "❌ Owner seulement.", msg);
        CONFIG.mode = CONFIG.mode === "both" ? "private" : "both";
        await reply(sock, from, `🌐 Mode: *${CONFIG.mode === "both" ? "PUBLIC" : "PRIVE"}*`, msg);
    },
    ...
    sudo: async (sock, from, msg, args, sender) => {
        ...
        await sock.sendMessage(from, { text: `✅ Sudo ajoute! @${shortNum(mentioned[0])} peut utiliser les commandes admin.`, mentions: mentioned }, { quoted: msg });
    },
...
async function runSecurityChecks(sock, from, msg, sender, body) {
    ...
    if (antiSpam) {
        ...
        if (data.count >= CONFIG.spamLimit) {
            ...
            await sock.sendMessage(from, { text: `🚫 @${shortNum(sender)} expulse pour spam.`, mentions: [sender] });
            return true;
        }
    }
    return false;
        ...
        for (const [key, session] of quizSessions) {
            if (key.includes(from) && key.includes(shortNum(sender))) {
                if (body.toLowerCase().includes(session.answer.toLowerCase())) {
                    ...
                    await reply(sock, from, `✅ *BONNE REPONSE!*\n\nReponse: *${session.correct}*\n💰 +50 coins! ⭐ +20 XP!`, msg);
                }
            }
        }
        return;
...
async function startOTechBot() {
    ...
    sock.ev.on("connection.update", async ({ connection, lastDisconnect }) => {
        ...
        if (connection === "close") {
            ...
            else { console.log("🔴 Session expiree. Supprime session_otech/ et relance."); process.exit(0); }
        }
    });
        ...
        try {
            ...
            for (const num of anu.participants) {
                ...
                if (anu.action === "add") {
                    ...
                    const byeText =
                        `👋 *${name}* (+${ns}) vient de quitter *${meta.subject}*.\n` +
